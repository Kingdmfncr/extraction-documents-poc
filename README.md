...
- ✅ **Tests** (`tests/test_extraction.py`) : 18 tests Pytest — normalisation des champs (date, montant, SIRET), comportement du moteur de patterns, règles de cohérence métier, cas limites (données incomplètes, SIRET manquant qui ne doit pas faire basculer le statut seul).
